...
def 关于():
    messagebox.showinfo(title='关于',message=f'ZouB项目（呼叫器应用终端模块化版）\n版本：Version 3.4.1.2503\n状态：公测\n所有教师：{接收用户名}')

def 修改设置():
    messagebox.showinfo(title='提示',message='该功能正在加紧赶制中，敬请期待……')
...
def 发送():
    ...
    if len(消息)!=0:
        #侦测所选班级
        if 一班.get():
            siot.publish_save(topic=一班端口,data=[输入框.get(),R,G,B])
            messagebox.showinfo(title='提示',message=f'{一班名称}消息发送成功！')
        if 二班.get():
            siot.publish_save(topic=二班端口,data=[输入框.get(),R,G,B])
            messagebox.showinfo(title='提示',message=f'{二班名称}消息发送成功！')
        if 三班.get():
            siot.publish_save(topic=三班端口,data=[输入框.get(),R,G,B])
            messagebox.showinfo(title='提示',message=f'{三班名称}消息发送成功！')
        if 四班.get():
            siot.publish_save(topic=四班端口,data=[输入框.get(),R,G,B])
            messagebox.showinfo(title='提示',message=f'{四班名称}消息发送成功！')
        if 五班.get():
            siot.publish_save(topic=五班端口,data=[输入框.get(),R,G,B])
            messagebox.showinfo(title='提示',message=f'{五班名称}消息发送成功！')
        if 六班.get():
            siot.publish_save(topic=六班端口,data=[输入框.get(),R,G,B])
            messagebox.showinfo(title='提示',message=f'{六班名称}消息发送成功！')
        if 七班.get():
            siot.publish_save(topic=七班端口,data=[输入框.get(),R,G,B])
            messagebox.showinfo(title='提示',message=f'{七班名称}消息发送成功！')
        if 八班.get():
            siot.publish_save(topic=八班端口,data=[输入框.get(),R,G,B])
...
siot.init(client_id="",server=IP,port=1883,user="siot",password="dfrobot")
siot.connect()
siot.loop()
